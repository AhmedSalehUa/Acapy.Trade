...
public class Offers {
    ...
    public static String getAutoNum() throws Exception {
        return db.get.getTableData("SELECT IFNULL(MAX(`id`)+1,1) FROM `sl_offers`").getValueAt(0, 0).toString();
    }

    public void getOfferDoc() throws Exception {

        File file = null;
        String selectSQL = "SELECT `doc` FROM `sl_offers` WHERE `id`='" + id + "'";
        JTable tab = db.get.getTableData("SELECT `doc_ext`,`date` FROM `sl_offers` WHERE `id`='" + id + "'");
