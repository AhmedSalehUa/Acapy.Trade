...
public class operation {
    ...
    public operation(int id, String client_name, String sales_name, String date, int total_cost, String pay_type) {
        this.id = id;
    ...
    public void setDoc_ext(String doc_ext) {
     ...
     public boolean Add() throws Exception {
      ...
      public boolean Edit() throws Exception {
       ...
       public static ObservableList<operation> getData() throws Exception {

        ObservableList<operation> data = FXCollections.observableArrayList();

        String SQL = "SELECT `cli_operation`.`id`,`cli_clients`.`name`, `sl_sales_members`.`name`,`cli_operation`.`date` ,`cli_operation`.`total_cost`, `cli_operation`.`pay_type` FROM `cli_operation`,`cli_clients`,`sl_sales_members`where `cli_operation`.`client_id`=`cli_clients`.`id`AND`cli_operation`.`sales_id`= `sl_sales_members`.`id`";
         // String SQL = "SELECT * FROM `cli_operation`";
        ResultSet rs = db.get.getReportCon().createStatement().executeQuery(SQL);

        while (rs.next()) {
            data.add(new operation (rs.getInt(1), rs.getString(2), rs.getString(3) , rs.getString(4),rs.getInt(5), rs.getString(6)));
        }
        return data;
    }
        ...
        JTable tab = db.get.getTableData("SELECT `doc_ext`,`date` FROM `cli_operation` WHERE `id`='" +  id + "'");
