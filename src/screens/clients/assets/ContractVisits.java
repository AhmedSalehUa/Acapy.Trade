...
public class ContractVisits {
    ...
    InputStream doc;
    String doc_ext;
    ClientScreenContractVisitsController s;
    ...
    public static ObservableList<ContractVisits> getData() throws Exception {
        ObservableList<ContractVisits> data = FXCollections.observableArrayList();
        ResultSet rs = db.get.getReportCon().createStatement().executeQuery("SELECT cli_contracts.`id`,mem_acapy_members.`name`, cli_contract_visits.`date`, cli_contract_visits.`report` FROM `cli_contracts`,`mem_acapy_members`,`cli_contract_visits` WHERE cli_contract_visits.`id`=cli_contracts.`id`AND`cli_contract_visits`.`id`= `mem_acapy_members`.`id`");
        while (rs.next()) {
