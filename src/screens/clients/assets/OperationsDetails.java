...
public class OperationsDetails {
    ...
    public boolean Add() throws Exception {
        PreparedStatement st = db.get.Prepare("INSERT INTO `cli_operation_details`(`id`, `operation_id`,`store_product_id`,`cost` ,`amount`, `total_cost`) VALUES (?,?,?,?,?,?)");
        st.setInt(1, id);
        st.setInt(2, operation_id);
        st.setInt(3, store_product_id);
        st.setString(4, cost);
    ...
    public boolean Edit() throws Exception {
        PreparedStatement st = db.get.Prepare("UPDATE `cli_operation_details` SET `operation_id`=?,`store_product_id`=? ,`cost`=? ,`amount`=?,`total_cost`=? WHERE `id`=?");

        st.setInt(1, operation_id);
        st.setInt(2, store_product_id);
        st.setString(3, cost);
